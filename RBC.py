...
class RBC: 
    ...
    def fit(self, X, y_label, y_reg, params_class = None, params_reg = None):
        
        if params_class is None:
            params_class = self.default_params_class
        if params_reg is None:
            params_reg = {}
            n_labels = len(set(y_label))
        ...
        self.classifier_method.fit(X, y_label)
    ...
    def predict(self, X_val):
        ...
        y_label_pred = self.classifier_method.predict(self.X_val)    
        #y_reg_pred = np.zeros(len(y_label_pred)).reshape(1,-1)
        y_reg_pred = np.empty(shape=(len(X_val),1))
        print(y_reg_pred.shape)
    
        for p in range(len(y_label_pred)):
            y_reg_pred[p] = self.regression_method_dic[y_label_pred[p]].predict(self.X_val[p].reshape(1,-1))
    ...
    def optimize_reg(self, regressor_label, hp_regressor, values_test , X, y_reg, y_label, fixed_hp = None, X_val = None, y_reg_val = None):
        ...
        for n in range(1, n_labels+1):
            ...
            y_label_dic[n] = np.empty(shape=(0,1))
            y_reg_dic[n] = np.empty(shape=(0,1))
             
        
        
        for i in range(len(y_label)):
            X_dic[y_label[i]] = np.append(X[i],X_dic[y_label[i]]).reshape(-1,self.n_features)
            #y_label_dic[y_label[i]] = np.append(y_label[i], y_label_dic[y_label[i]])
            y_reg_dic[y_label[i]] = np.append(y_reg[i], y_reg_dic[y_label[i]])
